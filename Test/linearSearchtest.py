...
def get_process_metrics(process_name):
    ...
    for proc in psutil.process_iter(['name', 'cpu_percent', 'memory_info', 'num_handles', 'num_threads']):
        if proc.info['name'] == process_name:
            ...
            return log_entry, memory_leak_detected, anomaly_report
...
while True:
    metrics, memory_leak_detected, anomaly_report = get_process_metrics(process_name)

    if memory_leak_detected:
        logging.warning(metrics)
        plot_graphs()  # Generate graph immediately if memory leak detected
    elif anomaly_report:
        logging.warning(f"Anomaly detected: {anomaly_report}")
